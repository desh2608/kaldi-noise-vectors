...
train_ivector_dir=exp/nnet3${nnet3_affix}/noise_${train_set}_sp_hires
...
if [ $type == "offline" ]; then 
  ...
  local/nnet3/extract_noise_vectors_online.sh \
    --stage $stage --nj $nj \
    --train-set $train_set --gmm $gmm \
    --test-sets "$test_sets" \
    --nnet3-affix "$nnet3_affix"
...
dir=${dir}_${type}
train_ivector_dir=${train_ivector_dir}_${type}
  ...
  for data in ${test_sets}; do
    ...
    for lmtype in tgpr_5k; do
      steps/nnet3/decode.sh \
        ...
        --online-ivector-dir exp/nnet3${nnet3_affix}/noise_${data}_hires_${type} \
