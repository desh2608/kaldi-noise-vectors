...
# Same as 1a but uses (offline or online) i-vectors.
...
ivector_type=offline  # offline/online
...
if [ $ivector_type == "offline" ]; then
  ivector_affix=
elif [ $ivector_type == "online" ]; then
  ivector_affix="_online"
else
  echo "Unknown ivector-type: ${ivector_type}" && exit 1
fi
